...
After running this function create_pkl_version()
you will have a version of your dataset with name {annot_all.pkl}
contains video_id /training examples {it's clips {each clip has 41 frames in the 
original dataset but i selected just 9 frames} 
{each clip has category which is the 
label of the clip , and it's selected 9 frames , each frame is an object of boxinfo class
class type }}
...
def load_tracking_annot(path):
    with open(path, 'r') as file:
        ...
        for idx, line in enumerate(file):
            ...
            # object has the info of the class
